...
![Raspberry Pi](https://img.shields.io/badge/Raspberry%20Pi-hôte%20self--hosted-A22846?logo=raspberrypi&logoColor=white)

Application web de suivi thyroïdien, inspirée de l'app Clue.  
**Stack** : TypeScript · Express · GraphQL (additif) · Prisma · PostgreSQL · React · Recharts · Docker
...
│   │   └── graphql/             # Endpoint POST /graphql, additif à la REST API — voir backend/docs/graphql.md
│   ├── docs/
│   │   └── graphql.md           # Pourquoi/comment du layer GraphQL (N+1, auth, hors scope)
│   ├── Dockerfile
│   └── package.json
│
├── frontend/
...
POST   /graphql                                   (additif à la REST ci-dessus, ne la remplace pas — voir backend/docs/graphql.md)
```
...
| API GraphQL | Apollo Server 5 + DataLoader (additif à REST, démo technique — voir [`backend/docs/graphql.md`](./backend/docs/graphql.md)) |
| ORM | Prisma 5 |
| BDD | PostgreSQL |
...
| Déploiement | Docker Compose (self-hosted, Raspberry Pi) |
| Reverse proxy | Traefik (partagé, `traefik-net`) |
| Accès public | cloudflared (tunnel) |
| BDD (infra) | PostgreSQL 17, instance partagée native (hors Docker) |

---

## 🤝 Contribuer

Voir [`CONTRIBUTING.fr.md`](./CONTRIBUTING.fr.md) pour l'environnement de développement, comment
reproduire la CI en local, et le format des PR. En cas de problème, [`TROUBLESHOOTING.fr.md`](./TROUBLESHOOTING.fr.md)
documente les incidents déjà rencontrés (et leur diagnostic) sur ce projet.

## 📝 Changelog

Les changements notables sont documentés dans [`CHANGELOG.md`](./CHANGELOG.md) (disponible en français uniquement).
